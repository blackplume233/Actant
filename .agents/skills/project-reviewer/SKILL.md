...
- 本轮审查是全局 review、专项 review 还是 issue 补充审查
- 需要覆盖的时间窗口、模块范围或 Roadmap 范围是否清楚
- 审查数据源是否可访问，例如 roadmap、tasks、issues、git 历史
...
- 关键数据源缺失，导致无法形成可信结论
- 发现的问题需要修改仓库内容才能表达，但当前 skill 只读
- 证据不足以支持严重结论

### 完成状态

- `DONE`: 所有核心发现都已映射为 issue、comment 或“无严重问题”结论
- `PARTIAL`: 完成了部分维度审查，但仍有范围未覆盖
- `BLOCKED`: 因数据缺失或范围不清无法继续

### 禁止行为

- 用印象代替证据
- 发现问题后只口头描述，不落到 issue/comment/明确结论
- 在只读审查中直接修改仓库文件
...
- 新问题：创建 Issue
- 已有问题：补充 Comment
- 未发现严重问题：明确输出 “no critical findings”
