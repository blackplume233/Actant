...
function createMockLocal(): ClientCallbackHandler {
  return {
    ...
    readTextFile: vi.fn().mockResolvedValue({ text: "local-content" }),
...
function createMockUpstream(): UpstreamHandler {
  return {
    ...
    sessionUpdate: vi.fn().mockResolvedValue(undefined),
    readTextFile: vi.fn().mockResolvedValue({ text: "ide-content" }),
    writeTextFile: vi.fn().mockResolvedValue({}),
...
describe("ClientCallbackRouter", () => {
  ...
  describe("without upstream (local mode)", () => {
    it("should route createTerminal to local", async () => {
      const result = await router.createTerminal({ command: "bash", args: [] });
    ...
    it("should route terminalOutput to local", async () => {
      const result = await router.terminalOutput({ terminalId: "t1" });
      expect(result.output).toBe("local-output");
      expect(local.terminalOutput).toHaveBeenCalledWith({ terminalId: "t1" });
    });

    it("should route waitForTerminalExit to local", async () => {
      const result = await router.waitForTerminalExit({ terminalId: "t1" });
      expect(result.exitCode).toBe(0);
      expect(local.waitForTerminalExit).toHaveBeenCalled();
    });

    it("should route killTerminal to local", async () => {
      await router.killTerminal({ terminalId: "t1" });
      expect(local.killTerminal).toHaveBeenCalledWith({ terminalId: "t1" });
    });

    it("should route releaseTerminal to local", async () => {
      await router.releaseTerminal({ terminalId: "t1" });
      expect(local.releaseTerminal).toHaveBeenCalledWith({ terminalId: "t1" });
    });
  ...
  describe("capability-based routing", () => {
    ...
    it("should use local fs when IDE declares no fs capability", async () => {
      ...
      const result = await router.readTextFile({ path: "/test.txt" });
      expect(result.text).toBe("local-content");
