...
export function createCatalogSyncCommand(client: RpcClient, printer: CliPrinter = defaultPrinter): Command {
  return new Command("sync")
    .description("Sync component catalog(s)")
    .argument("[name]", "Catalog name (syncs all if omitted)")
    .action(async (name?: string) => {
      try {
        const result = await client.call("catalog.sync", { name });
