...
function showOverview(): void {
  ...
  const lines = [
    ...
    `    ${chalk.cyan("actant init")}                      Create actant.namespace.json`,
    `    ${chalk.cyan("actant hub status")}                Activate the current project`,
    `    ${chalk.cyan("actant namespace validate")}        Validate the active namespace`,
    `    ${chalk.cyan("actant vfs mount list")}            Inspect declared mounts`,
    `    ${chalk.cyan("actant vfs mount add --type hostfs --path /extra --host-path ./extra")}`,
    "",
