...
**Plugin** 在此上下文中指 **Cloud Code Plugin**——即安装到 Agent workspace 中用于扩展 Agent 能力的插件包。不同于 Actant 自身的可插拔扩展（Phase 4 #14），这里的 Plugin 是 **Agent 侧的能力扩展**，不等同于 actant-side `PluginHost` 的系统插件。
```
Plugin 类型对照：
