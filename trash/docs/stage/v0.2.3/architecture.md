# Actant v0.2.3 — Architecture Document
...
├── mcp-server/   @actant/mcp-server  MCP server integration (stub)
...
- **Hook Types** (v0.2.3): `HookEventName`, `HookDeclaration`, `HookAction`, `HOOK_CATEGORIES`, `BUILTIN_EVENT_META` — unified 6-layer event taxonomy
...
- **AgentManager** — The central orchestrator for agent lifecycle
  ...
  - Archetypes (v0.2.3): `tool`, `employee`, `service` — determines default launch mode and autoStart
...
`BackendManager` adds: `registerAcpResolver`, `getAcpResolver`, `registerBuildProviderEnv`, `getBuildProviderEnv`, `registerBuilder`, `getBuilder`, `supportsMode`, `requireMode`, `checkAvailability`, `getInstallMethods`.
...
├── resolvePackage (v0.2.3: npm package for auto-install)
└── materialization (MaterializationSpec)

BackendManager (behavior, non-serializable)
├── acpResolvers: Map<string, AcpResolverFn>
├── buildProviderEnvFns: Map<string, BuildProviderEnvFn>
...
| `pi` | resolve, acp | in-process | First-class builtin (v0.2.3), ACP bridge |
...
- **CommunitySource** (v0.2.3) — Agent Skills Open Standard compatible repositories
...
- **DeclarativeBuilder** (v0.2.3) — Driven by `MaterializationSpec` on `BackendDefinition`; supports `single-file`, `per-file`, `dual` for skills
...
- **PiBuilder** — `.pi/` config files
- **Handlers**: `SkillsHandler`, `PromptsHandler`, `McpServersHandler`, `WorkflowHandler`, `PluginsHandler`
...
Unified 6-layer event taxonomy (v0.2.3):

| Layer | Category | Events |
|-------|----------|--------|
| System | actant | `actant:start`, `actant:stop`, `actant:error` |
| Entity | agent | `agent:created`, `agent:destroyed`, `agent:started`, `agent:stopped`, `agent:error` |
...
- **HookEventBus** — Central pub/sub; `on`, `off`, `emit` with permission-based `EmitGuard`
- **HookRegistry** — Registers workflow hooks at actant-level or instance-level; filters by `allowedCallers`
- **HookCategoryRegistry** — Extensible event taxonomy; plugins can add categories via `HookCategoryDefinition`
- **ActionRunner** — Executes `ShellAction`, `BuiltinAction`, `AgentAction` with template interpolation

#### Scheduler (`src/scheduler/`)

- **EmployeeScheduler** — Per-agent scheduler for heartbeat, cron, hook triggers; auto-initialized on agent start (v0.2.3)
- **TaskDispatcher** — Priority queue, sequential execution
- **ExecutionLog** — Per-agent execution history
- **InputSources**: `HeartbeatInput` (intervalMs), `CronInput` (pattern), `HookInput` (eventName)
- Emits `idle` event when queue drains

#### Provider (`src/provider/`)

- **ModelProviderRegistry** — Manages AI provider configs
- **ProviderEnvResolver** — Backend-aware provider env injection (v0.2.3)
...
65 RPC methods across 11 handler groups:
...
- Registers Pi backend with ACP resolver and builder
- Initializes EmployeeScheduler per agent on start
...
- **BinaryResolver** — Resolves ACP binary from PATH or `resolvePackage` fallback

### 6. `@actant/pi` — Lightweight In-Process Agent

- **PiBuilder** — Workspace builder for Pi agents (skills, prompts, workflow, permissions → `.pi/`)
...
AgentManager.createAgent()
  ...
  │       │   ├── Resolve builder (backend-specific or DeclarativeBuilder)
  │       │   ├── SkillsHandler → AGENTS.md / .claude/ / .pi/
  │       │   ├── PromptsHandler → prompts in workspace
  │       │   ├── McpServersHandler → .cursor/mcp.json
  │       │   └── WorkflowHandler → workflow config
...
AgentManager.runPrompt()
  ...
  │   ├── BuildProviderEnv (inject API keys, model config)
  ...
  │   └── PiCommunicator: createPiAgent → prompt → result
...
HookEventBus.emit(eventName, context, agentName?, data?)
  ...
  │
  ├── Instance-level listeners (matching agentName)
  │   └── HookRegistry.registeredWorkflows[agentName]
  │
  └── Actant-level listeners (no agentName filter)
      └── HookRegistry.registeredWorkflows[global]
          │
          ▼
      ActionRunner.runActions(actions, payload, ctx)
...
**Agent Archetypes (v0.2.3):**

| Archetype | Default Launch Mode | AutoStart | Description |
|-----------|-------------------|-----------|-------------|
| `tool` | `direct` | `false` | On-demand tool agents |
| `employee` | `acp-background` | `true` | Persistent background workers |
| `service` | `acp-service` | `true` | Always-on services with restart |
...
├── archetype: 'tool' | 'employee' | 'service' (v0.2.3)
├── autoStart: boolean (v0.2.3)
├── domainContext
...
- Daemon with 65 RPC methods
- Backend as VersionedComponent (BackendManager) with declarative builder
- Backend availability checking + install methods + auto-install
- Pi promoted to first-class builtin backend
- Agent archetypes (tool, employee, service) with autoStart
- Backend-aware provider env injection
...
- ACP employee tools capability (RFC #178)
- Agent memory system (Phase 5 design)

### Known Limitations

- MCP server package is a stub
- Backend `existenceCheck` requires the binary to be in PATH
- Community source validation is basic (no deep semantic checks)
