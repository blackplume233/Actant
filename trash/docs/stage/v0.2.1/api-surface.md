...
> **版本**: v0.1.0 | **生成时间**: 2026-02-24T08:30:57.966Z
...
| `create` | — | Create a new agent from a template | `<name>` | `--launch-mode <mode>`, `--work-dir <path>`, `--workspace <path>`, `--overwrite`, `--append`, `-f, --format <format>`, `-t, --template <template>` * |
...
| `resolve` | — | Resolve spawn info for an agent (external spawn support) | `<name>` | `-t, --template <template>`, `-f, --format <format>` |
...
| 子命令 | 别名 | 说明 | 参数 | 选项 |
|--------|------|------|------|------|
| `add` | — | Register a component source | `<url-or-path>` | `--type <type>`, `--branch <branch>`, `--name <name>` * |
...
| `validate` | — | Validate all assets in a component source | `[name]` | `--path <dir>`, `-f, --format <format>`, `--strict`, `--compat <standard>` |
