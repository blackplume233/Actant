...
> **版本**: v0.1.0 | **生成时间**: 2026-02-27T05:58:39.374Z
...
| `sessionId` | `string` | 是 |
| `types` | `string[]` | 否 |
| `Pagination` | `skip first N records. */ offset?: number` | 是 |
| `Pagination` | `max records to return (default 200). */ limit?: number` | 是 |
...
| 字段 | 类型 | 必需 |
|------|------|------|
| `events` | `Array<{ ts: number` | 是 |
| `event` | `string` | 是 |
| `agentName` | `string` | 否 |
| `caller` | `string` | 是 |
| `payload` | `Record<string, unknown>` | 是 |
