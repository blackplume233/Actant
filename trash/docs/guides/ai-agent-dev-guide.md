...
| ContextFS 架构基线 | `docs/design/contextfs-architecture.md` |
...
### 9.2 新增一个 Source 或资源类型

旧的 DomainContext 扩展路线已经退出当前主线。新增资源类型时先按 ContextFS 基线设计：

1. 先阅读 `docs/design/contextfs-architecture.md`
2. 再阅读 `docs/design/actant-vfs-reference-architecture.md`
3. 明确它属于哪种 `Source`
4. 明确它支持哪些 `Capability`
5. 明确它挂载到什么路径
6. 明确它是否受 `Project` 权限边界约束
7. 明确它是否需要 `watch` 或 `stream`
...
| [ContextFS 架构](../design/contextfs-architecture.md) | ContextFS 产品层对象模型 |
| [VFS Kernel 架构](../design/actant-vfs-reference-architecture.md) | VFS Kernel 分层与实现边界 |
