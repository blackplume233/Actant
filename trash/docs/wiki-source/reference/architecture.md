...
<!-- Manually maintained since v0.3.0 governance alignment (#278). Previous auto-generated content from update-wiki-from-stage.mjs preserved below with updates. -->

# Actant v0.3.0 — Architecture Document

> **Actant** 是面向 AI Agent 的**底层平台** — 负责工作区物化、运行时管理、协议桥接和自治能力承载。
> 它不是单一 Agent 产品或 Agent 管理器，而是支撑上层 Agent App / SOP 自动化 / CI 任务代理 / 引擎集成的统一基础设施。
>
> Docker 类比仍有助于理解分层（Dockerfile↔Template, Container↔Instance, Daemon↔Daemon），但不是产品主叙事。

## Platform Conceptual Model

### 管理深度递进 (Archetype Progression)

```text
repo  ──────►  service  ──────►  employee
工作区承载      运行时管理（主交付形态）  自治增强层
```

- **repo**: 物化模板、维护工作区，不要求自治运行
- **service**: 默认主交付形态 — 进程生命周期、会话、keepAlive、被动响应
- **employee**: 在 service 基础上叠加调度、心跳、事件驱动与自治执行

### 分层模型 (Layer Separation)
...
│  AgentManager · PluginHost · Scheduler · ContextInjector   │
│  ActantChannel · RecordSystem · VFS · BudgetManager        │
│  决定声明如何被物化、启动、调度、恢复和注入                 │
└────────────────────────────────────────────────────────────┘
```

> **Plugin 术语注意**: `domainContext.plugins` 中的 plugin 是 **agent-side 扩展**（安装到 workspace）；
> `PluginHost` 管理的是 **actant-side system plugin**（运行于平台 runtime），二者不是同一层概念。
...
├── channel/      @actant/channel     ActantChannel abstraction (v0.3.0)
├── channel-claude/ @actant/channel-claude  Claude Agent SDK adapter (v0.3.0)
├── tui/          @actant/tui         Unified TUI layer (v0.3.0)
├── api/          @actant/api         Daemon, socket server, RPC handlers
├── cli/          @actant/cli         CLI frontend (commander-based)
├── acp/          @actant/acp         ACP protocol adapter (now external-only)
...
**Agent Archetypes — Management Depth Progression:**

| Archetype | 平台定位 | Default Launch Mode | AutoStart | Canvas | ToolScope | Description |
|-----------|---------|-------------------|-----------|--------|-----------|-------------|
| `repo` | 工作区承载 | `direct` | `false` | No | `all` only | 物化模板、维护工作区，不要求自治运行 |
| `service` | **主交付形态** | `acp-service` | `true` | **Yes** | `all` + `service` | 进程生命周期、会话、keepAlive、被动响应；**默认产品与集成目标** |
| `employee` | 自治增强层 | `acp-background` | `true` | **Yes** | All scopes | 在 service 基础上叠加调度、心跳、事件驱动；**不是默认主交付形态** |

> `service` 是当前最稳定、最通用的交付形态，应优先作为产品默认目标。`employee` 是 `service + autonomy`，而非独立替代品。
...
- **Agent archetypes** (v0.2.5): `repo / service / employee` — management depth progression with hierarchical ToolScope
...
- **Dashboard i18n** — react-i18next framework with English + Chinese support
- **Live Canvas** — Service and employee agents push HTML via MCP tool; repo-rejected gating
- **SessionContextInjector** — Dynamic MCP tool injection with ToolScope hierarchy
- **Built-in MCP Server** — `actant_canvas_update`, `actant_canvas_clear` tools
- **Activity system** — Conversation assembly with prompt_sent/prompt_complete
- **ActantPlugin system + PluginHost** (#14) — Six-plug interface, dependency-ordered init, tick loop, HeartbeatPlugin builtin
- **SystemBudgetManager** — Service agent runtime tracking, dynamic keepAlive, auto-stop on expiry
- Cross-platform install scripts (Linux/macOS + Windows PowerShell)
- VitePress wiki documentation site

### v0.3.0 Additions

- **ActantChannel abstraction** (#279) — Unified communication layer, replaces direct ACP coupling; `@actant/acp` becomes external-only adapter
- **`@actant/channel-claude`** — Claude Agent SDK adapter implementing ActantChannel
- **`@actant/tui`** — Unified TUI layer based on `ActantChatView` / `StreamingMarkdown` / `VirtualTerminal`
- **RecordSystem** — Merged EventJournal + ActivityRecorder into unified record infrastructure
- **ChannelPermissions** — Protocol-level permission enforcement for ActantChannel
- **VFS (Virtual Filesystem)** — Registry-based virtual filesystem across runtime surfaces
- **Structured errors** — Unified error hierarchy across RPC and ACP layers
- **Centralized JSON-RPC transport** — Shared socket transport core
- **Security hardening** — Path traversal, shell injection, resource leak fixes
- **15 review issues resolved** — Structured errors, unified RPC, security hardening, cross-layer tests
...
- ACP-EX protocol extensions (#280–#290)
...
> **Updated**: 2026-03-16 | v0.3.0 | #278 governance alignment
