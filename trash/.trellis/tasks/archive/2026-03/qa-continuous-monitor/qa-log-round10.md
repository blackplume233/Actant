...
| 22 | agent status rw-agent-1 | **WARN** | status 仍显示 running（stop 后应有 stopped，可能时序） |
...
| 37 | agent list | **WARN** | 表仅显示 2 个（期望 3），可能时序 |
...
| 步骤 | 命令 | 判定 | 说明 |
|------|------|------|------|
| +1 | template load rw-pi-tpl | PASS | Loaded rw-pi-tpl@1.0.0 |
| +2 | agent create rw-pi-1 -t rw-pi-tpl | PASS | backendType: pi, status: created |
| +3 | agent resolve rw-pi-1 | PASS | Backend: pi, Command: acp-bridge.js |
| +4 | agent destroy rw-pi-1 --force | PASS | Destroyed |
