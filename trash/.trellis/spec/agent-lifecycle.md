...
> **若代码行为与本文档不一致，以本文档为准。**
>
> **⚠️ 历史说明**：本文档保留 Agent 生命周期相关内容，但当前架构基线已切换到 `ContextFS` / `VFS Kernel`。若本文档与 `vision.md`、`docs/design/contextfs-architecture.md`、`docs/design/actant-vfs-reference-architecture.md` 冲突，以新的 ContextFS 基线为准。
...
> **统一通信层例外**：对 `service` archetype，"实例已运行"通常意味着共享 runtime 已就绪，应优先通过 lease/shared-session 复用该 runtime，而不是把"occupied"默认解释为需要创建 ephemeral 副本。ephemeral 副本仅用于显式隔离、兼容性回退或非共享路由。
...
> 历史事件系统设计文档已移入 `trash/`。当前以本规范和 `communication-layer.md` 为准。
...
> - `service` 无调度器 —— 不允许配置 `schedule`，不初始化 EmployeeScheduler；它是共享 runtime communication target，默认通过 lease / shared session semantics 对外提供通信
> - `service` 的 `running` 目标语义是**communication-ready**，而不只是 process-alive；默认路由、readiness 字段与 facade 规则以 `communication-layer.md` 为准
> - `employee` 必须有 `schedule` 配置 —— 至少包含 heartbeat，是"有使命感的服务"；其外部/内部通信模型仍与统一通信层保持一致
...
| Chat | ✅（按需 acp direct） | ✅ | ✅ | repo 通过 acp direct 按需连接；service 通过 lease/session 语义连接共享 runtime；employee 沿用统一通信层但保留其自治运行约束 |
...
> 历史 subsystem 设计文档已移入 `trash/`。当前以本规范中的约束为准。
...
| Actant 角色 | spawn + 心跳监控 + 崩溃自动重启 + 维护 shared runtime readiness + 接受外部 prompt / lease / proxy 接入 |

**产品语义**：`service` 是当前主交付形态下的**共享 runtime communication target**。对外应优先表现为稳定的 Actant runtime facade；即使底层 backend 是 `claude-code`，外部调用者也应首先感知到的是 service、lease、conversation 与 readiness，而不是 backend bridge 细节。

**场景**：
- 7×24 运行的共享 Agent 服务
- CLI / Dashboard / 外部 ACP Client 共用的默认承载面
- 未来内部 agent-to-agent communication 的共享目标
...
# Agent 持续运行...
# 崩溃？Actant 自动重启（指数退避）
# Dashboard / agent.prompt / proxy 默认复用该 shared runtime
```

**通信约束**：
- `running` 的目标语义是 communication-ready，而不是仅进程存在
- 对运行中的 service，`proxy` 默认应走 lease-first 共享语义
- `agent.prompt` 是运行中 service 的正式 prompt surface
- `agent.run` 可保留为兼容/映射路径，但不是 shared service 的首要通信契约
- occupied service instance 不应默认退化为 ephemeral process fallback
...
| 并发客户端 | 通常单客户端 | 共享 runtime + lease/session 复用 |
| 运行状态含义 | process-liveness 为主 | **communication readiness 为主** |
...
### 9.1 单宿主进程与 `hub` 自举面（2026-03）

当前阶段，Actant 的自举路径遵循 **single daemon host + CLI-first hub surface**：

- 正式入口是 `actant hub <subcommand>`；
- `acthub <subcommand>` 只是别名，行为必须与 `actant hub` 完全一致；
- `hub` 不是第二个 daemon，而是单个 daemon host 上的 bootstrap capability surface；
- 当宿主未运行时，`hub` 命令可以自动复用或拉起最小化 bootstrap profile，但不得偷偷创建第二个长期进程。

#### 启动配置分层

| 配置 | 默认用途 | 必须加载 | 明确禁止默认激活 |
|------|---------|---------|----------------|
| `bootstrap` | CLI-first 自举、项目上下文发现、Hub 命令 | Host Kernel、Hub Core、Project Context Registry、Source/Config Loader、只读 VFS | `AgentService`、Scheduler、平台级 Kernel Agents、MCP bridge |
| `runtime` | 在 bootstrap 基础上按需启用运行时能力 | `bootstrap` 全部 + 被明确请求的 runtime modules | 未被请求的自治服务 |
| `autonomous` | 平台自治运行 / 长驻协作 | `runtime` 全部 + `AgentService` + 调度与平台级 Agent | — |

#### 激活规则

- 注册 runtime module ≠ 激活 runtime service；
- 加载 Agent Runtime 框架 ≠ 默认实例化 `AgentService`；
- `hub` 命令不得通过依赖链隐式拉起 `AgentService`；
- 任何需要完整 runtime 的能力，都必须走显式激活路径，而不是借 `hub` / project-context discovery 的名义偷偷升级宿主配置。

---

## 10. 平台级 Agent 体系（可选运行时层）

> 详细设计见 [Hub Agent Kernel (#204)](https://github.com/blackplume233/Actant/issues/204)

Actant 可以自带一套三层平台级 Agent，它们是 actant-hub 的初始内容，不是用户创建的应用级 Agent，而是构成平台"自治操作系统内核"的组成部分。

> **当前阶段约束**：该体系属于可选运行时层，不是 bootstrap profile 的默认负载。默认自举不得因为启动 `hub` 或读取项目上下文而自动启用这些 Agent。
...
|------|-----------|------------|-----------------|------|
| Kernel | `employee` | `acp-service` | `persistent` | 仅在 `autonomous` profile 中随 daemon 启动，7×24 运行 |
| Auxiliary | `employee` | `acp-service` | `persistent` | 按需激活或由 Kernel 委派 |
| Spark | `employee` | `acp-service` | `persistent` | 贡献者专用，自主 fork/PR，默认不启用 |

### 10.2 Kernel Agent 职责

| Agent | 核心职责 | 关键交互 |
|-------|---------|---------|
| **Steward（总管）** | 在自治 profile 中作为人类交互统一入口；与 CLI 并存而不是取代 bootstrap 阶段 CLI | Email in → 理解意图 → 委派给其他 Agent |
