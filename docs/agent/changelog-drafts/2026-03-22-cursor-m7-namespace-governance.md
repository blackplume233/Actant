...
- 新增 `actant init` 与 `actant namespace validate`，并把 `actant vfs mount add/remove/list` 收口为 namespace authoring surface
- 移除 `actant setup` 与旧的 runtime/config fallback 入口
...
- 仍持有旧配置的仓库需要人工改写 `actant.namespace.json` 后再进入新的运行时加载路径
...
- 结果：`lint` / `type-check` / `test` 全部通过；新增行模式扫描未发现 `console.log`、TypeScript `any` 或非空断言；terminology gate 通过；M7 roadmap 已勾满并切换到 M8
