...
- 直接删除 `packages/catalog` 包及其 CLI/RPC/REST/plugin 入口，不再把 catalog 当作活跃模块边界。
- 扩展 terminology gate，把已删除的 `catalog`/`preset` 边界、`agent-runtime` 插件口径和死掉的兼容导出路径纳入活跃门禁。
