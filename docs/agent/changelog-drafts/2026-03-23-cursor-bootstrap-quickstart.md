# Bootstrap Quickstart Closure

## 变更摘要

- 将仓库根默认运行时配置切到 `actant.namespace.json`，删除活跃入口中的旧 `actant.project.json`。
- 将 smoke / E2E 自举验证统一到 `test:bootstrap`，并让 `test:e2e` 直接串联 namespace 自举链路。
...
- 新增 `pnpm test:bootstrap` 作为最短自举验证入口。
...
- `test:context` 仍保留，但现在只是 `test:bootstrap` 的兼容别名。
