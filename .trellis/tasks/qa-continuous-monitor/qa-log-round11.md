...
| 11 | agent create -t bad-template | PASS | exit 1, RPC -32001 Template not found |
...
| 22 | agent status rw-agent-1 (after stop) | PASS | status: stopped（R10 为 WARN，本次改善） |
| 23 | restart flow (start + stop) | WARN | agent not found on start，可能 daemon 预暖环境导致 |
| 24 | agent stop rw-agent-1 | WARN | not found（承接 23，agent 已不存在） |
...
| 37 | agent list | WARN | 表仅显示 2 个（期望 3），与 R10 一致 |
