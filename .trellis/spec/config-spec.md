...
### Hub 激活挂载（`hub.activate` 后）

> Hub 采用单宿主命名空间，不创建第二个 daemon。当前项目上下文在宿主内部统一挂载到 `/hub/...`，并可在项目切换时热替换。

| 挂载点 | 源类型 | 生命周期 |
|--------|--------|---------|
| `/hub/project` | `project-context` | `manual`（Hub 当前激活项目） |
| `/hub/workspace` | `filesystem` | `manual` |
| `/hub/config` | `filesystem` | `manual` |
| `/hub/skills` | `registry/filesystem` | `manual` |
| `/hub/prompts` | `registry/filesystem` | `manual` |
| `/hub/mcp` | `registry/filesystem` | `manual` |
| `/hub/workflows` | `registry/filesystem` | `manual` |
| `/hub/templates` | `registry/filesystem` | `manual` |
...
## 14. `actant.project.json` - 项目上下文引导配置（2026-03）

> 本节定义当前阶段用于 CLI-first Hub、自举宿主与 MCP detached-readonly 模式的最小配置面。它不替代全局 `config.json`、模板配置或 daemon 运行时配置；它只负责告诉 Actant “当前项目有哪些本地上下文、额外来源和引导入口”。

### 14.1 定位

`actant.project.json` 位于项目根目录，用于支持以下场景：

- 让 `actant hub ...` / `hub.activate` 能在单宿主上激活当前项目上下文
- 让 `@actant/mcp-server` 在无 daemon 时仍可进入 standalone project-context 模式
- 为 Agent / IDE / 外部客户端提供统一的项目上下文入口 `/project/context.json`
- 声明当前项目应优先暴露的本地 `configs/` 目录与额外组件来源

当前版本刻意保持最小，不承担以下职责：

- 不声明运行时 session、schedule、process、agent lifecycle
- 不替代 `AgentTemplate` / `AgentAppManifest`
- 不承担通用 source manager 的全部能力
...
```json
{
  "version": 1,
  "name": "agent-craft",
  "description": "Project bootstrap context for Actant MCP",
  "configsDir": "configs",
  "sources": [
    {
      "name": "shared",
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `version` | `1` | 否 | 当前配置版本。省略时按 `1` 解释 |
| `name` | `string` | 否 | 项目显示名；默认取项目根目录名 |
| `description` | `string` | 否 | 项目说明，供 `/project/context.json` 暴露 |
| `configsDir` | `string` | 否 | 本地 Actant 组件目录，相对项目根目录解析；默认 `configs` |
| `sources` | `ProjectSourceEntry[]` | 否 | 额外组件来源列表 |
| `entrypoints` | `ActantProjectEntrypoints` | 否 | 供 backend 发现项目知识和建议读取顺序的显式入口 |

### 14.2a `ActantProjectEntrypoints`

```ts
interface ActantProjectEntrypoints {
  readFirst?: string[];
  knowledge?: string[];
}
```
...
约束：
- `readFirst` 为项目根目录相对路径数组，声明 backend 在读取 `/project/context.json` 后建议继续读取的文件顺序
- `knowledge` 为项目根目录相对路径数组，声明当前项目的知识 / spec 入口文件
- loader 会将它们解析为绝对路径并放入 `/project/context.json`
- 若声明的文件不存在，必须作为 warning 出现在 `/project/context.json`

### 14.3 `ProjectSourceEntry`

```ts
interface ProjectSourceEntry {
  name: string;
  config: SourceConfig;
}
```
...
- `name` 必须在当前项目内唯一
- `config` 复用 `SourceConfig` 联合类型，当前允许 `local` / `github` / `community`
- `local.path` 相对 `actant.project.json` 所在项目根目录解析
- 外部 source 加载后的组件名称必须以 `sourceName@componentName` 命名空间注入，避免污染本地项目作用域

### 14.4 行为约定

当 `actant.project.json` 存在时，project-context loader（无论被 `hub.activate` 还是 detached MCP 调用）必须：

- 尝试读取本地 `configsDir` 下的 `skills`、`prompts`、`mcp`、`workflows`、`templates`
- 按顺序加载 `sources`
- 将 source 加载失败记录为 warning，而不是让整个项目上下文初始化失败
- 解析 `entrypoints.readFirst` / `entrypoints.knowledge`，并将缺失文件记录为 warning
- 生成稳定的项目上下文挂载；在 Hub host 中挂到 `/hub/...`，在 detached-readonly 模式下挂到 `/project` 等只读逻辑根

当它被 `hub.activate` 用于 CLI-first 自举时，还必须满足：

- 不因读取项目上下文而默认激活 `AgentService`、Scheduler、平台级 Kernel Agents
- 允许宿主复用同一进程并在项目切换时替换当前 `/hub/...` 挂载

当文件不存在或校验失败时：

- MCP 仍应继续启动 standalone 模式
- 缺省配置视为 `{ "version": 1 }`
- 校验错误与 source 错误必须进入 `/project/context.json` 的 warning 面

### 14.5 与 VFS 的关系

Hub connected 模式下，`actant.project.json` 驱动的宿主内部挂载面为：
...
standalone / detached-readonly 模式下，`actant.project.json` 驱动的最小挂载面为：

- `/project`
- `/workspace`
- `/config`（若 `configsDir` 存在）
- `/skills`
- `/prompts`
- `/workflows`
- `/templates`
- `/daemon`（standalone 虚拟信息源）

其中 `/project` 至少暴露以下文件：

- `/project/context.json`
- `/project/actant.project.json`
