...
> 本节定义当前阶段用于 MCP 自举与项目级上下文暴露的最小配置面。它不替代全局 `config.json`、模板配置或 daemon 运行时配置；它只负责告诉 Actant “当前项目有哪些本地上下文、额外来源和引导入口”。
...
当 `actant.project.json` 存在时，standalone project-context 模式必须：
...
standalone 模式下，`actant.project.json` 驱动的最小挂载面为：
